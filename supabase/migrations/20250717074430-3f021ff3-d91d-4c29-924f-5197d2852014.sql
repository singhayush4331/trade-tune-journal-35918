...
-- Create permissions for the new roles
...
-- Wiggly-only users get limited permissions (no Academy access)
INSERT INTO public.role_permissions (role_id, permission, granted)
SELECT r.id, p.permission, true
FROM public.roles r
CROSS JOIN (
...
-- Update the user signup trigger to handle new role types
...
BEGIN
    ...
    IF signup_type = 'haven_ark_masterclass' THEN
        role_name := 'haven_ark_masterclass';
    ELSIF signup_type = 'wiggly_only' THEN  
        role_name := 'wiggly_only';
    ELSIF signup_type = 'academy' THEN
